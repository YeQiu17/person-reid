    ...
    def _initialize_cameras(self, camera_details):
        ...
        for i, camera in enumerate(camera_details['cameraDetails']):
            ...
            self.camera_trackers[camera_id] = DeepSort(
                max_age=50, n_init=5, nms_max_overlap=0.6, max_cosine_distance=0.3, nn_budget=100
            )
...
class UserCameraProcessor:
    def __init__(self, user_id, camera_details, db_connection_string):
        ...
        self.failed_cameras = {}  # Track failed cameras in memory
    ...
    async def _validate_stream(self, url, index):
        ...
        try:
            ...
            if not cap.isOpened():
                ...
                camera_id = self.tracker.index_to_camera_id[index]
                await asyncio.to_thread(
                    self.tracker.db.store_camera_status,
                    self.user_id,
                    camera_id,
                    url,
                    "failed"
                )
                self.failed_cameras[camera_id] = {"url": url, "status": "failed"}
                return False, None
            ret, _ = cap.read()
            if not ret:
    ...
    async def _initialize_cameras(self):
        ...
        for i, url in enumerate(self.tracker.camera_urls):
            is_valid, cap = await self._validate_stream(url, i)
    ...
    async def _capture_frame_with_retry(self, cap, camera_id):
        ...
        video_url = self.tracker.camera_urls[list(self.tracker.index_to_camera_id.values()).index(camera_id)]
        await asyncio.to_thread(
            self.tracker.db.store_camera_status,
            self.user_id,
            camera_id,
        ...
        self.failed_cameras[camera_id] = {"url": video_url, "status": "failed"}
        return False, None
    ...
    async def _process_cameras(self):
        ...
        caps = await self._initialize_cameras()
    ...
    def get_failed_cameras(self):
        """Return the list of failed cameras."""
        return self.failed_cameras
...
class MultiUserTrackingSystem:
    ...
    async def get_failed_cameras(self, user_id):
        """Get the list of failed cameras for a user."""
        if user_id not in self.user_processors:
            return {}
        processor = self.user_processors[user_id]
        return processor.get_failed_cameras()
