...
class MultiCameraTracker:
    ...
    def is_moving_towards_zone(self, center, previous_position, door_zone):
        """Check if the movement direction indicates entering or exiting the zone."""
        (x_min, y_min), (x_max, y_max) = door_zone
        if previous_position:
            prev_x, prev_y = previous_position
            cur_x, cur_y = center
            if cur_y > y_max and prev_y <= y_max:
                return "entry"  # Moving into the zone
            elif cur_y < y_min and prev_y >= y_min:
                return "exit"  # Moving out of the zone
        return None
    ...
    async def process_frame(self, frame, camera_id):
        ...
        if results and len(results[0].boxes) > 0:
            for box in results[0].boxes:
                ...
                conf = float(box.conf)
                if conf < self.detection_confidence or (x2 - x1) * (y2 - y1) < 100:
                    continue
        ...
        for track in tracks:
            ...
            # Entry/Exit logic with improved checks
            ...
            movement = self.is_moving_towards_zone(center, previous_position, door_zone)
            if movement == "entry":
