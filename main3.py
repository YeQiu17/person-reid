...
class MultiCameraTracker:
    ...
    def _compute_similarity(self, features1, features2):
        """
        Compute a more robust similarity metric using multiple techniques:
        - Cosine similarity 
        - Euclidean distance
        - Weighted combination for more nuanced matching
        """
        if features1 is None or features2 is None:
            return 0

        # Normalize features to handle potential scaling differences
        features1 = features1 / (np.linalg.norm(features1) + 1e-10)
        features2 = features2 / (np.linalg.norm(features2) + 1e-10)
        ...
        # Combine metrics with weighted approach
        # Lower distance means more similar, so we invert it
        normalized_dist = 1 / (1 + euclidean_dist)
        
        # Weighted combination: favor cosine similarity more
    ...
    def _hash_features(self, features):
        """
        Convert feature array to a hashable representation.
        Uses a tuple of rounded float values to create a consistent hash.
        """
        if features is None:
            return None
        # Round to reduce floating-point precision variations
        return tuple(np.round(features, decimals=4))

    async def _find_or_create_person(self, features, camera_id):
        """
        Enhanced person matching with robust feature tracking.
        """
        if features is None:
            return None

        # Convert features to a hashable form
        hashed_features = self._hash_features(features)
        
        best_match_id = None
        best_match_score = -1
        
        # Dynamically adjust match threshold
        dynamic_threshold = max(0.6, min(0.85, self.match_threshold * 1.1))

        # Retrieve or initialize feature history for this hash
        if hashed_features not in self.feature_history:
            self.feature_history[hashed_features] = []

        # Comprehensive search for best match
        for person_id, person_data in self.persons.items():
            # Compare against stored features
            stored_features = person_data["features"]
            
            # Compute similarity
            primary_similarity = self._compute_similarity(features, stored_features)
            
            # Compare with historical features for temporal consistency
            historical_similarities = [
                self._compute_similarity(features, hist_feature) 
                for hist_feature in self.feature_history.get(hashed_features, [])
            ]
            
            # Compute average historical similarity
            avg_historical_similarity = (
                sum(historical_similarities) / len(historical_similarities) 
                if historical_similarities else primary_similarity
            )
            
            # Combine current and historical similarities
            combined_similarity = 0.6 * primary_similarity + 0.4 * avg_historical_similarity
        ...
        if best_match_score > dynamic_threshold:
            ...
            # Update person's feature history and camera visibility
            self.persons[best_match_id]["history"].add(camera_id)
            
            # Store the original features, not the hashed version
            self.feature_history[hashed_features].append(features)
            
            return best_match_id

        # Create new person if no good match found
