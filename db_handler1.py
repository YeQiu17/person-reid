...
class ReIDDatabase:
    def __init__(self, connection_string, database_name="occupancydb", container_name="person_features",
                 ...
                 user_logs_container_name="user_logs", camera_status_container_name="camera_status"):
        ...
        self.camera_status_container = self.database.create_container_if_not_exists(
            id=camera_status_container_name,
            partition_key=PartitionKey(path="/user_id"),
        )
    ...
    def store_camera_status(self, user_id, camera_id, video_url, status):
        """Store the status of a camera for a user in the camera_status container."""
        try:
            # Create a unique ID for the camera status document
            doc_id = f"{user_id}_{camera_id}"
            timestamp = datetime.now(timezone.utc).isoformat()
            ...
            self.camera_status_container.upsert_item(doc)
            
            self.logger.info(f"Stored camera status for user {user_id}, camera {camera_id}: {status}")
            return True
        except exceptions.CosmosHttpResponseError as e:
            self.logger.error(f"Failed to store camera status for user {user_id}, camera {camera_id}: {str(e)}")
            return False